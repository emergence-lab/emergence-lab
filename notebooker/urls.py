...
urlpatterns = patterns('',
    ...
    #url(r'^edit/(?P<notebook_name>[a-z0-9\w-]+)/(?P<cell>\d+)$', login_required(never_cache(views.CellEdit.as_view())), name='notebook_cell_edit'),
    url(r'^embedded$', login_required(never_cache(views.EmbeddedNotebook.as_view())), name='notebook_embedded'),

    url(r'^edit/(?P<notebook_name>[A-Za-z0-9\-]+)$', login_required(never_cache(views.NotebookInteractive.as_view())), name='notebook_int'),

    url(r'^view/(?P<notebook_name>[a-z0-9\w-]+)$', login_required(views.NotebookViewer.as_view()), name='notebook_viewer'),
