...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'operator.user'
        db.add_column(u'operators', 'user',
                      self.gf('django.db.models.fields.related.OneToOneField')(default=1, to=orm['auth.User'], unique=True),
                      keep_default=False)

        # Adding M2M table for field projects on 'operator'
        m2m_table_name = db.shorten_name(u'operators_projects')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('operator', models.ForeignKey(orm[u'core.operator'], null=False)),
            ('project', models.ForeignKey(orm[u'core.project'], null=False))
        ))
        db.create_unique(m2m_table_name, ['operator_id', 'project_id'])


    def backwards(self, orm):
        # Deleting field 'operator.user'
        db.delete_column(u'operators', 'user_id')

        # Removing M2M table for field projects on 'operator'
        db.delete_table(db.shorten_name(u'operators_projects'))
